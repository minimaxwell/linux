...
struct phy_mux_port {
	...
	/* The port cannot tell its own state, only the parent or the mux can
	 * know it through an inband mechanism
	 */
	bool inband_detect;
...
struct phy_mux_config {
	...
	/* The mux itself can isolate ports, allowing multiple exclusive ports to coexist */
	u16 isolating:1;

	/* Ports that have inband link detection (they can't tell their own
	 * state, only the parent can) can simultaneously detect on that mux */
	u16 multi_inband:1;

	/* Some muxes have internal HW logic to perform the port selection (some
	 * multi-port PHYs call that auto-media detection).
	 */
	u16 can_auto_select:1;
};
