...
static int mode_canal(struct spi_device *spi, struct codec *codec, int canal, int val)
{
	...
	_Result += codec->delta_em;
	_Result = ~(_Result - 1) + VAL_BUS_NUM;
...
static int niveau_em(struct device *dev, int canal, const char *buf, size_t count)
{
	...
	_Val += codec->delta_em;
	if (codec->debug)
		dev_info(dev, "delta em = %d\n", _Val);
	_Val = ~(_Val - 1) + VAL_BUS_NUM;
	if (codec->debug)
...
static int niveau_rec(struct device *dev, int canal, const char *buf, size_t count)
{
	...
	if (codec->debug)
		dev_info(dev, "lecture = %d\n", _Val);
	_Val -= codec->delta_rec;
	if (codec->debug)
		dev_info(dev, "delta rec = %d\n", _Val);
	_Val -= VAL_BUS_NUM;
	if (codec->debug)
...
static int __devinit codec_probe(struct spi_device *spi)
{
	...
	const char *name_codec = NULL, *delta_em = NULL, *delta_rec = NULL;
	...
	if (np) {
		...
		delta_em = of_get_property(np, "delta_em", &codec->delta_em);
		delta_rec = of_get_property(np, "delta_rec", &codec->delta_rec);
	}
	if (!name_codec || (len < sizeof(*name_codec))) {
		_Result = -ENODEV;
		goto err;
	}
	if (!delta_em || (codec->delta_em < sizeof(*delta_em)))
		codec->delta_em = 0;
	else
		codec->delta_em = gain_val(delta_em, codec->delta_em);
	if (!delta_rec || (codec->delta_rec < sizeof(*delta_rec)))
		codec->delta_rec = 0;
	else
		codec->delta_rec = gain_val(delta_rec, codec->delta_rec);
	len = 0;
