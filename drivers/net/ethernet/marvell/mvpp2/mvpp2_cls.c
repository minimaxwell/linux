				      ...
				      struct mvpp2_rfs_rule *rfs_flow)
{
	...
	for (i = 0; i < MVPP2_N_PRS_FLOWS; i++) {
		...
		/* Make sure we iterate only once per classifier flow */
		...
		pr_info("%s : Inserting rule for prs_flow %d flow %d at id %d\n", __func__, i, flow->flow_id, fe.index);
