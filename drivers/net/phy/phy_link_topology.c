			  ...
			  enum phy_upstream upt, void *upstream)
{
	...
	if (phy->phy_port) {
		ret = phy_link_topo_add_port(topo, phy->phy_port);
		if (ret) {
			pr_info("%s : error inserting port\n", __func__);
			goto remove_phy;
		}
	}
			   ...
			   struct phy_device *phy)
{
	...
	if (phy->phy_port)
		phy_link_topo_del_port(topo, phy->phy_port);
...
int phy_link_topo_add_port(struct phy_link_topology *topo,
...
void phy_link_topo_del_port(struct phy_link_topology *topo,
			    struct phy_port *port)
{
