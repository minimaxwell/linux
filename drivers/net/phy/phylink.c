...
{
	unsigned long caps = linkmode_caps_max_speed(max_speed);
				       ...
				       unsigned long mac_capabilities)
{
	...
	caps = linkmode_interface_get_capabilities(state->interface,
						   mac_capabilities,
						   state->rate_matching);
	linkmodes_from_caps(mask, caps);
			      ...
			      const struct fwnode_handle *fwnode)
{
	...
	if (pl->cfg_link_an_mode == MLO_AN_INBAND) {
		...
		case PHY_INTERFACE_MODE_XLGMII:
			linkmodes_from_interface(pl->link_config.interface,
						 RATE_MATCH_NONE,
						 pl->supported);
			break;
			    ...
			    struct phylink_link_state link_state)
{
	...
	case RATE_MATCH_PAUSE:
		 ...
		 */
		speed = linkmode_interface_max_speed(link_state.interface);
		duplex = DUPLEX_FULL;
		rx_pause = true;
		break;
