...
static int phy_port_select_start(struct phy_mux_port *port)
{
	return phy_port_generic_start_listening(port);
}
...
struct phy_mux_port *phy_mux_port_create_from_phy(struct phy_device *phy)
{
	...
	if (phy_can_isolate(phy)) {
		pr_info("%s : PHY can isolate, using isolating ops\n", __func__);
		port->ops = &phy_isolate_ops;
	} else {
		pr_info("%s : PHY can't isolate, using power ops\n", __func__);
		port->ops = &phy_power_ops;
	}

	port->inband_detect = false;
...
void phy_mux_notify(struct phy_device *phydev, bool on)
{
	...
	pr_info("%s : PHY %s reports link %s\n", __func__, phydev_name(phydev),
		on ? "up" : "down");
