...
static int
ethnl_port_fill_reply(const struct ethnl_req_info *req_base, struct sk_buff *skb)
{
	...
	if (req_info->port_id) {
		port = phy_link_topo_get_port(req_base->dev->link_topo,
					      req_info->port_id);
	} else if (req_base->phydev) {
		port = phy_get_single_port(req_base->phydev);
	...
	if (!phy_port_get_state(port, &state)) {
		if(nla_put_u8(skb, ETHTOOL_A_PORT_ENABLED, state.enabled) ||
		   nla_put_u8(skb, ETHTOOL_A_PORT_FORCED, state.forced) ||
		   nla_put_u8(skb, ETHTOOL_A_PORT_LINK, state.link) ||
		   nla_put_u32(skb, ETHTOOL_A_PORT_SPEED, state.speed))
			return -EMSGSIZE;
	}

	if (nla_put_u32(skb, ETHTOOL_A_PORT_ID, port->id) ||
	    nla_put_u32(skb, ETHTOOL_A_PORT_TYPE, port->parent_type))
		return -EMSGSIZE;
...
int ethnl_port_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	ret = ethnl_port_reply_size(&req_info.base, info->extack);
	if (ret < 0)
		goto err_unlock_rtnl;
	...
	ret = ethnl_port_fill_reply(&req_info.base, rskb);
	if (ret)
		goto err_free_msg;
				   ...
				   struct netlink_callback *cb)
{
	...
	xa_for_each_start(&dev->link_topo->ports, ctx->port_id, port, ctx->port_id) {
		...
		ret = ethnl_port_fill_reply(&pri->base, skb);
		if (ret) {
			genlmsg_cancel(skb, ehdr);
			break;
		}
...
static int ethnl_set_port(struct ethnl_req_info *req_info, struct genl_info *info)
{
	...
	} else if (req_info->phydev) {
		port = phy_get_single_port(req_info->phydev);
	...
	ethnl_update_bool(&state.enabled, tb[ETHTOOL_A_PHY_LOOPBACK], &mod);
